...
function displayImagesFromJson() {
    fetch('./data.json')  // Fetch the JSON file
        .then(response => response.json())  // Parse the JSON
        .then(data => {
            // Access a key from the JSON and update the image src
            document.getElementById('navBarImage').src = data.fan_ally_logo_light;

            document.getElementById('headshotImage').src = data.ryan_rockey_headshot;

            document.getElementById('favicon').href = data.fan_ally_logo_dark;
...
document.addEventListener('DOMContentLoaded', displayImagesFromJson);
let savedTeams = sessionStorage.getItem('savedTeams') || '';
if (savedTeams) {
